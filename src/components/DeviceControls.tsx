...
import { RingPlayerEffectControls } from "./controls/RingPlayerEffectControls";
...
export const DeviceControls: React.FC<DeviceControlsProps> = ({ deviceId, onDisconnect }) => {
	...
	const [activeEffectTab, setActiveEffectTab] = useState<EffectTab>(effectTabs[0]);
	...
	return (
		<Card
			...
			radius="md"
			withBorder
			className={styles.deviceControlsContainer}
		>
			<Stack gap="md">
				...
				{ipAddress ? (
					<WebSocketConnection srDriver={srDriver} ipAddress={ipAddress} />
				) : (
				...
				<SegmentedControl value={activeEffectTab} onChange={setActiveEffectTab} data={effectTabs} />

				<div className={styles.controlsContainer}>
					{activeEffectTab === "flat" && <FlatColorControls srDriver={srDriver} />}

					{activeEffectTab === "rainbow" && <RainbowColorControls srDriver={srDriver} />}

					{activeEffectTab === "blend" && <ColorBlendControls srDriver={srDriver} />}

					{activeEffectTab === "twinkling" && <TwinkleEffectControls srDriver={srDriver} />}

					{activeEffectTab === "rain" && <RainEffectControls srDriver={srDriver} />}

					{activeEffectTab === "waves" && <WaveEffectControls srDriver={srDriver} />}

					{activeEffectTab === "pulse" && <PulsePlayerControls srDriver={srDriver} />}

					{activeEffectTab === "ring" && <RingPlayerEffectControls srDriver={srDriver} />}
				</div>
