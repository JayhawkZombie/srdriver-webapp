...
import { useDeviceControllerContext, useHeartbeatStatus } from '../controllers/DeviceControllerContext';
import { Device } from '../types/Device';

const DeviceConnectionRow: React.FC<{ device: Device; connectDevice: (id: string) => void; disconnectDevice: (id: string) => void; }> = ({ device, connectDevice, disconnectDevice }) => {
  ...
  return (
    <Box key={device.id} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
      ...
      <AnimatedStatusChip
        ...
        icon={<FavoriteIcon fontSize="small" color={heartbeat?.isAlive ? 'error' : 'disabled'} />}
      />
      {device.isConnected ? (
  ...
  return (
    <div>
      {devices.map((device: Device) => (
        <DeviceConnectionRow key={device.id} device={device} connectDevice={connectDevice} disconnectDevice={disconnectDevice} />
      ))}
    </div>
  );
};
