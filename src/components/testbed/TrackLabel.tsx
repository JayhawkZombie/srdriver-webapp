...
const TrackLabel: React.FC<TrackLabelProps> = React.memo(({
  name, type, color, isEditing, editingValue, editingType, onEdit, onEditCommit, onEditChange, onTypeChange
}) => {
  const trackType = TRACK_TYPES.find(tt => tt.type === (isEditing ? editingType : type));
  const Icon = trackType?.icon;
  const labelColor = color || trackType?.color || '#90caf9';
  return isEditing ? (
    <div style={{ width: LABEL_WIDTH - 10, marginBottom: TRACK_GAP, display: 'flex', alignItems: 'center', gap: 4 }}>
      <input
        type="text"
        value={editingValue}
        autoFocus
    ...
    >
      {Icon ? <Icon style={{ fontSize: 18, marginRight: 2, verticalAlign: 'middle' }} /> : null}
      {name}
    </div>
  );
});
