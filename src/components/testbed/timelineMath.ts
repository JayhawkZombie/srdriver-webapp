...
export function timeToX({ time, duration, width }: { time: number; duration: number; width: number }) {
  return (time / duration) * (width - TIMELINE_LEFT - TIMELINE_RIGHT_PAD) + TIMELINE_LEFT;
}

// Converts X coordinate to time (seconds)
export function xToTime({ x, duration, width }: { x: number; duration: number; width: number }) {
  return ((x - TIMELINE_LEFT) / (width - TIMELINE_LEFT - TIMELINE_RIGHT_PAD)) * duration;
}
...
export function trackIndexToCenterY(trackIdx: number) {
  ...
  return 40 + trackIdx * (TRACK_HEIGHT + TRACK_GAP) + (TRACK_HEIGHT - 16) / 2 + 8;
}
...
export function centerYToTrackIndex(centerY: number, numTracks: number) {
  ...
  const idx = Math.round((centerY - 40 - (TRACK_HEIGHT - 16) / 2 - 8) / (TRACK_HEIGHT + TRACK_GAP));
 ...
 */
export function mainRectWidth(x1: number, x2: number, dragLeftX: number | null, dragRightX: number | null) {
  if (dragRightX !== null) return dragRightX - (dragLeftX !== null ? dragLeftX + HANDLE_OFFSET : x1) + HANDLE_OFFSET;
