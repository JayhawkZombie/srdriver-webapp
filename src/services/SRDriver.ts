...
export class SRDriver {
	...
	async initialize(): Promise<void> {
		...
		this.brightnessCharacteristic = await service.getCharacteristic(
			"4df3a1f9-2a42-43ee-ac96-f7db09abb4f0"
		);
	...
	async getIPAddress(): Promise<string | null> {
		if (!this.ipAddressCharacteristic && !this.wsIP) {
			console.log(
				"📱 IP address characteristic not available (older device) or WebSocket IP not available"
			);
			return null;
		}
	...
	async sendWebSocketCommand(command: any): Promise<void> {
		...
		if (!this.wsConnection || this.wsConnection.readyState !== WebSocket.OPEN) {
			console.error(
				`📱 WebSocket: ❌ Not connected (readyState: ${this.wsConnection?.readyState})`
			);
		...
		try {
			...
			console.log(
				`📱 WebSocket: ✅ Command sent successfully in ${(endTime - startTime).toFixed(2)}ms`
			);
		} catch (error) {
